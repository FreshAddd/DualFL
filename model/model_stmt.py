...
from model.ModelSTMT import ModelSTMT
...
class TrainModel_STMT(pl.LightningModule):
    def __init__(self, input_embed_d=768, output_embed_d=500):
        super().__init__()
        self.input_embed_d = input_embed_d
        self.output_embed_d = output_embed_d
        self.model = ModelSTMT(input_embed_d=self.input_embed_d, output_embed_d=self.output_embed_d, heads=64)
    ...
    def forward(self, passed_test_cases, failed_test_cases, call_method, belong_method, stmt, cfg, dfg, ast):
        ...
        x = self.model(passed_test_cases, failed_test_cases, call_method, belong_method, stmt, cfg, dfg, ast)
        return x

    def training_step(self, batch, batch_idx):
        batch_size = int(batch[-1].shape[0])
        passed_test_cases, failed_test_cases, call_method, belong_method, stmt, cfg, dfg, ast, labels, stmt_id, method_id = batch
        preds = self(passed_test_cases, failed_test_cases, call_method, belong_method, stmt, cfg, dfg, ast)
