...
class AggModel_Method(torch.nn.Module):
    def __init__(self, input_embed_d=768, output_embed_d=500,
                 ...
                 num_call_stmt_attr_cases_attr=3,
                 num_internal_stmt_attr_cases_attr=3,
                 heads=4
                 ):
        super(AggModel_Method, self).__init__()
        ...
        self.num_call_stmt_attr_cases_attr = num_call_stmt_attr_cases_attr
        self.num_internal_stmt_attr_cases_attr = num_internal_stmt_attr_cases_attr
        ...
        self.call_stmt_attr_agg_model = AttrAggModel(input_embed_d=self.input_embed_d,
                                                   output_embed_d=self.output_embed_d,
                                                   num_agg=self.num_call_stmt_attr_cases_attr)
        self.call_stmt_neighbor_agg_model = NeighborAggModel()
        
        self.internal_stmt_attr_agg_model = AttrAggModel(input_embed_d=self.input_embed_d,
                                                       output_embed_d=self.output_embed_d,
                                                       num_agg=self.num_internal_stmt_attr_cases_attr)
        self.internal_stmt_neighbor_agg_model = NeighborAggModel()
        
        self.vector_agg_model = VectorAggModel(embed_d=self.output_embed_d, num_agg=7, heads=heads)
    ...
    def forward(self, passed_test_cases, failed_test_cases, history_change, call_information, methods, call_stmt, internal_stmt):
        ...
        call_stmt = self.call_stmt_attr_agg_model(call_stmt)
        internal_stmt = self.internal_stmt_attr_agg_model(internal_stmt)
        
        call_stmt = self.call_stmt_neighbor_agg_model(call_stmt)
        internal_stmt = self.internal_stmt_neighbor_agg_model(internal_stmt)
        ...
        vectors = self.vector_agg_model(
            [passed_test_cases, failed_test_cases, history_change, call_information, call_stmt, internal_stmt],
            methods
        )
...
class VectorAggModel(torch.nn.Module):
    def __init__(self, embed_d=500, num_agg=7, heads=64):
