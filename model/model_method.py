...
from model.ModelMethod import ModelMethod
...
class TrainModel_Method(pl.LightningModule):
    def __init__(self, input_embed_d=768, output_embed_d=500):
        super().__init__()
        self.input_embed_d = input_embed_d
        self.output_embed_d = output_embed_d
        self.model = ModelMethod(input_embed_d=self.input_embed_d, output_embed_d=self.output_embed_d, heads=64)
        self.example_input_array = (
            torch.zeros((5, 10, 2, 768)), 
            torch.zeros((5, 9, 2, 768)), 
            torch.zeros((5, 8, 3, 768)), 
            torch.zeros((5, 7, 3, 768)), 
            torch.zeros((5, 1, 3, 768)),
            torch.zeros((5, 6, 3, 768)),  
            torch.zeros((5, 5, 3, 768))   
        )
        self.best_FR = 9999
    ...
    def forward(self, passed_test_cases, failed_test_cases, history_change, call_information, methods, call_stmt, internal_stmt):
        ...
        x = self.model(passed_test_cases, failed_test_cases, history_change, call_information, methods, call_stmt, internal_stmt)
        return x

    def training_step(self, batch, batch_idx):
        batch_size = int(batch[-1].shape[0])
        passed_test_cases, failed_test_cases, history_change, call_information, methods, call_stmt, internal_stmt, labels, method_id = batch
        preds = self(passed_test_cases, failed_test_cases, history_change, call_information, methods, call_stmt, internal_stmt)
